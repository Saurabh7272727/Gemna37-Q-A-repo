...
export default function AttendancePage({ renderPart, tokenexpire }) {
    ...
    const submitHandler = async (setOpen) => {
        try {
            const result = await api.postRequest('/api/attendance', {
                gemidlog: `${userInformation?.ref_id?._id}`,
            ...
            if (result.success && result.status === 200) {
                ...
                result.status === 429 ||
                result.status === 409 ||
                result.status === 500 || result.status === 422
            ) {
            ...
            } else if (result.status === 202 && !result.success) {
                setShow(true);
                sessionStorage.setItem('sessionToken', result.sessionToken)
            }
        } catch (error) {
    ...
    return (
        <>
            {
                error?.error && <NotifyError message={error?.message} />
            }
            {
                show && <StudentAttendanceForm />
            }
                ...
                <section className="relative max-w-7xl mx-auto px-6 py-12 text-center overflow-hidden">
                    ...
                    <section className="max-w-8xl mx-auto px-6 py-10">
                        ...
                        <MagicButton
                            ...
                            text="Use me" css={'md:w-[220px] cursur-pointer w-[200px]'} />
                    </section>
                </section>
